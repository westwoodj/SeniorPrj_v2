...
def update_D():
    ...
    D_caret_2 = eta * neg(DL.dot(p).dot(p.T)) + eta * pos(yL.dot(p.T)) + beta * neg(L21.dot(U)) + beta * neg(L22.dot(DL))
...
def update_U():
    ...
    U_tilde = alpha*(Y*(U.dot(T).dot(U.T))).dot(U).dot(T.T) + alpha*(Y*(U.dot(T).dot(U.T))).T.dot(U).dot(T) + lmbda*U + beta*pos(L11.dot(U)) + beta*pos(L12.dot(DL))
